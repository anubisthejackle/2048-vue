<template>
    <div
        :style="{transform: `${tilePosition}` }"
        class="tile"
        :class="`${dynamicClass}`"
    >
        <div class="tile-inner">{{ tileValue }}</div>
    </div>
</template>
    ...
    export default {
        ...
        computed: {
            tilePosition() {
                ...
                // return value should look like `translate(COLUMN PIXELS, ROW PIXELS)`
                let column = ((this.tileColumn - 1) * 121);
                let row = ((this.tileRow - 1) * 121);
